...
def data_iterator(filename,dictionary=None):
    with codecs.open(filename,encoding="utf-8",errors='ignore') as f:
        for line in f:
            if dictionary==None:
                yield line.strip()
            else:
                yield [dictionary[word] if word in dictionary else 0 for word in line.strip().split()]

def build_dataset1(filename,min_count =2):
    count = dict()
    count[unknown_token]=min_count+1
    dictionary = dict()
    dictionary [unknown_token] = 0
    reversed_dictionary = [unknown_token]
    for line in data_iterator(filename):
...
count,dictionary,reverse_dictionary = build_dataset1(filename,min_count=25)
vocabulary_size = len(reverse_dictionary)
def generate_batch1(batch_size,  num_skips,skip_window, num_steps=1):
    global dictionary
    x,y=[],[]
    context_candidates = [w for w in range(2 * skip_window + 1) if w != skip_window]
    for xx in range(num_steps):
        for line_tokens in data_iterator(filename,dictionary):
...
if True:
  ...
  for step in xrange(5):
    for step_i,(batch_inputs, batch_labels) in enumerate( generate_batch1(batch_size, num_skips, skip_window)):
...
try:
  ...
  plot_only = len(reverse_dictionary) if len(reverse_dictionary) < 400 else 400
