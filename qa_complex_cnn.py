...
from dataset import complex_qa as qa
from models.classification import tensorflow as models
from tools import evaluation
from dataset.complex_qa import QAHelper as helper
...
datas=reader.get_train(overlap_feature=False)
for data in datas:
    print(data)
exit()
...
#    sess = tf.Session(config=session_conf)
    ...
    for i in range(params.epochs):
        print(i)
        for data in reader.get_train(overlap_feature=False):
            print(data)
            exit()
            _, step, loss, accuracy = model.train(sess,data)
